    ...
    view! {
        <section id=(format!("run-{run_id}-inspector")) class="min-w-0 rounded-panel border border-border bg-surface p-4 shadow-panel" aria-labelledby=(format!("inspector-title-{run_id}"))>
            <div class="flex items-start justify-between gap-4">
                <div><p class="text-xs font-semibold uppercase tracking-[0.04em] text-text-muted">"Selected run"</p><h2 class="text-xl font-semibold" id=(format!("inspector-title-{run_id}"))>"Execution route"</h2></div>
                <p class="rounded-control border border-current px-2 py-1 text-sm font-semibold text-text-secondary data-[status=Running]:text-status-warning data-[status=Completed]:text-status-healthy data-[status=Failed]:text-status-error" data-status=(run_status(&run)) role="status" aria-live="polite">(run_status(&run))</p>
            </div>
            <dl class="my-4 grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,3fr)_minmax(7rem,1fr)]">
                <div><dt class="text-xs font-semibold text-text-muted">"Workflow"</dt><dd class="mt-1 break-anywhere font-mono text-[0.8125rem]">(run.workflow_id.clone())</dd></div>
                <div><dt class="text-xs font-semibold text-text-muted">"Trigger"</dt><dd class="mt-1 break-anywhere font-mono text-[0.8125rem]">(trigger(&run))</dd></div>
                <div><dt class="text-xs font-semibold text-text-muted">"Input"</dt><dd class="mt-1 break-anywhere font-mono text-[0.8125rem]">(run.input.summary())</dd></div>
                <div><dt class="text-xs font-semibold text-text-muted">"Elapsed"</dt><dd class="mt-1 font-mono text-[0.8125rem]">(elapsed(&run))</dd></div>
                <div class="lg:col-span-2"><dt class="text-xs font-semibold text-text-muted">"Route"</dt><dd class="mt-1 break-normal font-mono text-[0.8125rem]" data-testid="route-summary">(run.route_summary.clone())</dd></div>
            </dl>
            if let Some(message) = error { <p class="mb-4 border-l-[3px] border-status-error bg-surface-elevated p-3" role="alert">(message)</p> }
            <div class="mb-3 flex flex-wrap gap-4 text-sm text-text-secondary" aria-label="Topology state legend"><span class="inline-flex items-center gap-2"><i class="size-3 rounded-full border border-text-muted"></i>"Idle"</span><span class="inline-flex items-center gap-2"><i class="size-3 rounded-full border border-status-warning bg-status-warning"></i>"Active"</span><span class="inline-flex items-center gap-2"><i class="size-3 rounded-full border border-status-healthy bg-status-healthy"></i>"Traversed"</span></div>
...
pub(super) async fn run_history(runs: Vec<RunSnapshot>) -> Result {
    view! {
        <section id="run-history-region" class="min-w-0 rounded-panel border border-border bg-surface p-4 shadow-panel" aria-labelledby="history-title">
            <div class="flex items-start justify-between gap-4"><div><p class="text-xs font-semibold uppercase tracking-[0.04em] text-text-muted">"Process lifetime"</p><h2 id="history-title" class="text-xl font-semibold">"Run history"</h2></div></div>
            <div class="max-w-full min-w-0 overflow-x-auto" tabindex="0" aria-label="Run history, horizontally scrollable">
                <table class="w-full min-w-[56rem] border-collapse text-sm"><thead><tr> for heading in ["Run ID", "Workflow", "Trigger", "Input", "Status", "Route", "Elapsed"] { <th class="border-b border-border p-3 text-left align-top text-xs font-semibold text-text-muted" scope="col">(heading)</th> } </tr></thead>
                    <tbody>
                        ...
                        for run in runs {
                            let run_id = run.run_id.to_string();
                            <tr data-attr:aria-current=(format!("$selectedRunId === '{run_id}' ? 'true' : 'false'")) class="aria-[current=true]:bg-surface-elevated">
                                <td class="border-b border-border p-3 align-top"><button class="border-0 bg-transparent p-0 font-mono text-[0.8125rem] text-accent-hover underline" type="button" data-on:click=(history_expression(&run))>(run_id)</button></td>
