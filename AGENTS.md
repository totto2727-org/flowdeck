...
- Active runs, terminal history, traces, graph-flow sessions, and schedule leases share one Toasty Turso database, in memory by default.
- A process-wide semaphore bounds concurrent drivers. History and graph-flow sessions have no application-imposed count limit or automatic deletion; database storage and UI rendering are separate from concurrency control.
