...
- SC-1: Runs with normalized start/finish timestamp columns, ordered step rows, graph sessions, and leases are stored in one Turso database rather than parallel application caches.
- SC-2: The unreleased initial migration initializes an empty database, and reopening preserves terminal history without replaying interrupted work. It provides no upgrade for earlier draft databases.
- SC-3: ORM rows and JSON boundary values are validated before domain construction, including multi-value consistency checks and derived-state reconstruction.
...
Storage, JSON conversion, concurrency, and recovery contracts use automated Rust assertions and integration scenarios against real SQLite.
...
Storage tests live in `src/storage_test.rs`, and row-conversion tests live beside their implementations under `src/storage/`.
...
| TC-003 | SC-3 | Persisted rows and parsed JSON values cross validated boundaries | ai-driven | inspection | Review normalized run, step, and session rows plus passing corruption, scalar, and collection-invariant tests | Workflow-owned JSON remains opaque after required shape checks |
...
| TC-008 | SC-5 | Retention and concurrency behavior is verified | ai-driven | inspection | Review passing SQLite tests for millisecond-plus-submillisecond timestamp precision, deterministic history ties, active-run retention, session CAS, and lease exclusion | No mocked database |
...
| TC-003 | Passed: normalized row-conversion tests, workflow input/configuration tests, and restored task-context tests reject malformed syntax, invalid scalar values, duplicate/incorrect trace identities, inconsistent lifecycles, and invalid paths. |
...
Existing migration, memory isolation, file reopening, recovery, retention, CAS, and row/domain validation checks pass against Turso.
...
### Normalized timestamp-column evidence, 2026-09-07
...
| History uses persisted start milliseconds rather than insertion or completion order | `history_sorts_by_start_timestamp_despite_insertion_and_completion_order` |
...
| Start and finish columns preserve independent millisecond-plus-submillisecond precision | `run_timestamp_columns_follow_snapshot_times_without_losing_snapshot_precision` and `terminal_insert_preserves_independent_start_and_finish_times` |
...
No user database was reset or deleted, and the unreleased rewritten initial migration deliberately adds no compatibility upgrade for earlier drafts.
...
## Normalized storage verification (2026-09-07)

| Requirement | Check and observed result |
| --- | --- |
| Fixed session, run, and step fields are columns, with JSON only for dynamic values | `normalized_run_and_session_columns_round_trip_nested_state_and_repeated_steps` passed against the real Turso database: ORM columns contain fixed metadata, nested input/context/state round-trip, and repeated steps retain their identities. |
| Derived traversal and duration preserve domain semantics | The same real database test passed exact traversal, step order, timestamp, and duration assertions. `run_timestamp_columns_preserve_submillisecond_precision` passed exact submillisecond reconstruction. |
| Parent and child mutations are atomic | `failed_step_insert_rolls_back_parent_and_child_changes` passed with an actual SQLite trigger aborting child insertion. The parent summary and steps remained unchanged. |
| Session concurrency and restart behavior remain unchanged | The real database session CAS/concurrency tests and file reopen/recovery tests passed, including preservation of more than 100 completed runs and sessions. |
| Real application behavior remains usable | Built application HTTP smoke passed dashboard GET, manual workflow submission, 11 live SSE patches from Running to Completed, completed-history reload, and invalid-input rejection without changing manual history. Run: `eccb3abd-b1b7-48c8-8bc3-9fdae6fc44de`. The owned server exited successfully on SIGTERM. |

The built-in browser bridge responded, but `list_tabs` returned no browser windows and `new_tab` failed with an unexpected error.
The HTTP checks above exercise the real server, but do not constitute browser or visual QA for this normalization change.
Earlier draft database schemas are intentionally incompatible with this unreleased initial migration and are rejected without deletion or automatic reset.

Final `nix develop -c just ci` passed after all normalization changes, including formatting, all-target/all-feature Clippy, asset bundling, workspace build, and the complete test suite.
`step_foreign_key_rejects_orphans_and_cascades_explicit_run_deletion` passed against Turso, confirming both orphan rejection and child-row cleanup on explicit parent deletion.
The row-conversion regressions also passed: inconsistent duration/traversal is rejected before serialization, and an invalid persisted step sequence is rejected during restoration.
